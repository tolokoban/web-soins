...
/**
 * Return a string with as many lines as patients:
...
function execStatus( $carecenterId ) {
    ...
    foreach( $patientIds as $patientId ) {
        ...
        $patients[] = $patient["key"] . ":" . $admissions;
    }
    return implode("\n", $patients);
}

function getAdmissions( $patientId ) {
    $stm = \Data\query("SELECT C.`uuid`, C.`version` FROM "
            . \Data\Admission\name() . ' AS A, '
            . \Data\Consultation\name() . ' AS C '
            . "WHERE A.`patient`=? "
    ...
    while( null != ($row = $stm->fetch()) ) {
        $admissions[] = $row["uuid"] . "," . $row["version"];
    }
    return implode(";", $admissions);
}
